...
static void glfwKeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    else if ((GLFW_MOD_CONTROL == mods) && ('A' == key))
    {
        LOGGER(logging::INFO) << "CTRL a - toggle animation";
        w->GetEngineWnd()->ToggleFlag(wflags::ANIMATE);
    }
    else if ((GLFW_MOD_CONTROL == mods) && ('C' == key))
    {
        LOGGER(logging::INFO) << "CTRL c - toggle feedback control";
        w->GetEngineWnd()->ToggleInvPendulumFlag(ge::pflags::CONTROL);
    }
    ...
    else if ((GLFW_MOD_CONTROL == mods) && ('L' == key))
    {
        LOGGER(logging::INFO) << "CTRL L - Load nn";
        w->GetEngineWnd()->GetInvPendulum().LoadNN();
    }
    else if ((GLFW_MOD_CONTROL == mods) && ('N' == key))
    {
        LOGGER(logging::INFO) << "CTRL N - toggle control nn";
        w->GetEngineWnd()->ToggleInvPendulumFlag(ge::pflags::CONTROL_NN);
    }
    ...
    else if ((GLFW_MOD_CONTROL == mods) && ('T' == key))
    {
        ...
        w->GetEngineWnd()->ToggleInvPendulumFlag(ge::pflags::NN_TRAIN);
    }
    ...
    else if ((GLFW_MOD_CONTROL == mods) && ('W' == key))
    {
        ...
        w->GetEngineWnd()->ToggleInvPendulumFlag(ge::pflags::VVERBOSE);
    }
    else {}
}
